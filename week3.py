...
def process_and_store_documents(folder_path, embedding_client, db_name="chunks.db"):
    """
    Belirtilen klasördeki dosyaları okur, chunk'lara böler ve veritabanına kaydeder.
    Veritabanı bağlantısını (conn) döndürür.
    """
    ...
    if len(docs) > 0:
        chunks_example = chunk_document(docs[0]["content"], docs[0]["file_name"])
        print(f"Chunk's number: {len(chunks_example)}")
        # print(chunks_example) # Çıktıyı kalabalıklaştırmaması için istenirse kapatılabilir
...
# Eğer sadece bu dosya test edilmek istenirse:
if __name__ == "__main__":
    ...
    # Veritabanını oluştur ve belgeleri yükle
    conn = process_and_store_documents("Belgeler", embedding_client)
